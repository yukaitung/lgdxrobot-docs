...
To create a profile, modify the source code of the `lgdxrobot2_navigation` package:

1. In the `lgdxrobot2_navigation/param` folder, copy any existing folder and rename it using a custom profile name. It is recommended that the name starts with `slam-` or `loc-`.
2. Edit the parameters in the copied folder. Parameters may be copied from the root of `lgdxrobot2_navigation/param`, but `ekf.yaml` and `nav2.yaml` are required.
3. (Optional) In the `lgdxrobot2_navigation/rviz` folder, copy any RViz configuration file and rename it to match the custom profile name.  
